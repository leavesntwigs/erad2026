...
{
   void RadxEvad (
     const size_t *sweep_index,  // array of starting ray number for each sweep 
     const size_t nsweeps,
     const float *rays, 
     const size_t nrays, // rays per sweep
     const float *elevs, // elevation for each sweep
     const float *azs,   // azimuth for each ray; same dimension and length as rays
     size_t  nGates,
     float startRangeKm,
     float gateSpacingKm,
...
extern "C" {
 int _mydoubleCompare(const void *i, const void *j)
 {
  double *f1 = (double *) i;
  double *f2 = (double *) j;
  if (*f1 < *f2) {
...
An xradar DataTree sweep group is a xarray.Dataset
HERE!!!! TODO: work out the conversion from xarray.Dataset/xradar:DataTree ==> base types for passing to RadxEvad-lite

sweep[0] = index of first ray in 1st sweep
sweep[1] = index of first ray in 2nd sweep
...
//  const static int nDivCoeff = 2;
  ...
  int _params_debug = 0;
  int Params_DEBUG_VERBOSE = 0;
  int Params_DEBUG_EXTRA = 3;
  int _params_slice_delta_azimuth = slice_delta_azimuth;
...
auto _computeMeanVel = [&](AzSlice &slice)

{

  if ((int) slice.pts.size() >= _params_min_vel_values_per_slice) {
    slice.valid = true;
  } else {
    slice.valid = false;
  }
...
auto _computeMedianVel = [&](int sliceNum)

{
  ...
  int nHalfMedian = _params_n_slices_for_vel_median / 2;
  ...
  qsort(vel, nMedian, sizeof(double), _mydoubleCompare);
...
//  if (_params_debug >= Params_DEBUG_EXTRA) {
...
auto _unfoldFromZeroIsodop = [&](int zeroIsodopIndex)
...
auto _unfoldFromPreviousRing = [&](double ht)
...

{
  ...
  for (int ii = 0; ii < _profileNLevels; ii++) {
    ...
    if (ptsFound.size() > 0) {
      ...
      ProfilePt ptBelow(missingVal);
      ProfilePt ptAbove(missingVal);
...
auto _computeVertVel = [&](double wtop)
...
auto _computeFf = [&](double az, double *ff)
...
auto _invertMatrix = [&](double *data, int nn) 
{
...

{
  ...
  for (int ilevel = 0; ilevel < _profileNLevels; ilevel++) {
    ...
    //if (_params_compute_profile_spacing_from_data) {
  ...
  
{
  ...
  if (_ring.fitRmsError > _params_max_fit_rms_error) {
    if (_params_debug >= Params_DEBUG_VERBOSE) {
      cerr << "Ring invalid, elev, minRange: "
           << _ring.elev << ", " << _ring.midRange << endl;
      cerr << "  max rms error allowed: " << _params_max_fit_rms_error << endl;
  ...
  int maxMissingAllowed = (int)
    (_params_max_missing_sector_size / _params_slice_delta_azimuth + 0.5);
  ...
  if (maxMissing > maxMissingAllowed) {
    if (_params_debug >= Params_DEBUG_VERBOSE) {
      ...
      cerr << "  max missing sector size: "
           << _params_max_missing_sector_size << endl;
...
auto _printResultsForRing = [&](ostream &out, const VelRing &ring)
...
//auto _doubleCompare = [&](const void *i, const void *j)
//{
//  double *f1 = (double *) i;
//  double *f2 = (double *) j;
//  if (*f1 < *f2) {
...
// lambda capture-default allows capture by reference all the local variables.
auto computeSolutionForRing = [&](
 size_t isweep, double elev, 
 const float *velArray,
 const size_t nrays,
 size_t nGates,
 const float *azs,
 const float nyquist)
{
  ...
  for (int islice = 0; islice < _nAzSlices; islice++) {
    ...
    printf("islice %d: ffs: %f %f %f %f %f %f %f\n", islice, slice.ff[0],  slice.ff[1], slice.ff[2], slice.ff[3], slice.ff[4], slice.ff[5], slice.ff[6]);
  ...
  for (size_t iray = 0; iray < nrays; iray++) { 
    ...
    double az = azs[iray]; // ray->getAzimuthDeg();
    if (az < 0) {
      az += 360;
    } else if (az > 360) {
      az -= 360;
    ...
    ngood++;
    // velMiss = missingVal;  // velFld->getMissingFl32();
    double rayNyquist = nyquist;      // ray->getNyquistMps();
   
    /* 
    const Radx::fl32 *censorArray = NULL;
    Radx::fl32 censorMiss = -9999;
    if (_params_censor_using_thresholds) {
      const RadxField *censorFld = ray->getField(_params_censor_field_name);
      if (censorFld == NULL) {
        if (_params_debug >= Params_DEBUG_VERBOSE) {
          cerr << "WARNING - _computeSolutionForRing()" << endl;
          cerr << "  Cannot find censor field, name: "
               << _params_censor_field_name << endl;
          cerr << "  Ray az: " << az << endl;
        }
      } else {
    ...
    */
    ...
    for (int igate = _ring.startGate; igate <= _ring.endGate; igate++) {
      ...
      //    accept = false;
      //  } else if (censor < _params_censor_min_value ||
      //             censor > _params_censor_max_value) {
      //    accept = false;
      //  }
      //}
      if (accept) {
...
// HERE!!!!!
  ...
  _sliceDeltaAz = _params_slice_delta_azimuth;
  ...
  if (_profileNLevels < 1) {
    ...
    cerr << "  profile_min_height: " << _profileMinHt << endl;
    cerr << "  profile_max_height: " << _profileMaxHt << endl;
    cerr << "  profile_height_interval: " << _profileDeltaHt << endl;
    OK = FALSE;
  }
  ...
  //nGates = _readVol.getMaxNGates();
  _radxStartRange = startRangeKm;
  _radxGateSpacing = gateSpacingKm;

  _radarLatitude = radarLatitudeDeg;
  _radarLongitude = radarLongitudeDeg;
  _radarAltitude = radarAltitude;
  ...
  //double _max_range 
   // _max_range = _params_max_range;
  double _min_range; //  = min_range;
  double _delta_range; //  = delta_range;
  //_nRanges = (int) ((max_range - min_range) /
  //  delta_range + 0.5);  

  //if (range_gate_geom_equal) {
  ...
  for (size_t isweep = 0; isweep < nsweeps; isweep++) {
	...
	_ring.startGate = 0;
      ...
      if (_ring.endGate > nGates - 1) {
	_ring.endGate = nGates - 1;
      }
...
//      }
      ...
      size_t rayIdx = 0;
      size_t lastRayIdx = sweep_index[isweep];
      if (isweep == 0) {
        rayIdx = 0;
      } else {
        rayIdx = sweep_index[isweep-1];
      }
// assume first ray index = 0, we cannot know the total number of rays
      printf("lastRayIdx = %zu, rayIdx = %zu, nsweeps = %zu\n", lastRayIdx, rayIdx, nsweeps);
      size_t nraysForSweep = lastRayIdx - rayIdx + 1;
if (nraysForSweep > 50 ) exit(1);
      if (computeSolutionForRing(isweep, elev, &rays[rayIdx], nraysForSweep, 
        nGates, azs, nyquist) == 0) {
        ngood++;
      }
...
//  }
  ...
  int _params_w_at_top_level = 0;
  _computeVertVel(_params_w_at_top_level);
